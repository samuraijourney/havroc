...
#define TCP_BUF_SIZE       1400

// Application specific status/error codes
typedef enum{
    // Choosing -0x7D0 to avoid overlap w/ host-driver's error codes
    UCP_CLIENT_FAILED = -0x7D0,
    TCP_SERVER_FAILED = UCP_CLIENT_FAILED - 1,
    DEVICE_NOT_IN_STATION_MODE = UCP_CLIENT_FAILED - 2,
...
//****************************************************************************
int UDP_Broadcast(unsigned short usPort, unsigned long *buf, unsigned short count);
int TCP_Receive(unsigned short usPort);
...
unsigned long  g_ulPacketCount = PACKET_COUNT;
unsigned long  g_ulIpAddr = 0;
unsigned long  g_cBsdBuf[UDP_BUF_SIZE];
...
int TCP_Receive(unsigned short usPort)
{
    ...
    // filling the buffer
    for (iCounter=0 ; iCounter<TCP_BUF_SIZE ; iCounter++)
    {
        g_cBsdBuf[iCounter] = (char)(iCounter % 10);
    }

    iTestBufLen  = TCP_BUF_SIZE;
...
int UDP_Broadcast(unsigned short usPort, unsigned long *buf, unsigned short count)
{
    ...
    {
        g_cBsdBuf[iCounter] = buf[iCounter];
    }
...
void main()
{
    ...
    ip_address[0]= SL_IPV4_BYTE(g_ulIpAddr,3);
    ip_address[1]= SL_IPV4_BYTE(g_ulIpAddr,2);
    ip_address[2]= SL_IPV4_BYTE(g_ulIpAddr,1);
    ip_address[3]= SL_IPV4_BYTE(g_ulIpAddr,0);

	// Run UDP client to broadcast IP address
    lRetVal = UDP_Broadcast(PORT_NUM, &ip_address[0], 4);
    if(lRetVal < 0)
    {
        ERR_PRINT(lRetVal);
        LOOP_FOREVER();
