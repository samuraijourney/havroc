...
unsigned long  		  UDP_BufferSize = 0;
unsigned long  		  TCP_BufferSize = 0;
unsigned long  		  UDP_Buffer[BUFF_SIZE];
char  		  		  TCP_SendBuffer[BUFF_SIZE];
...
long WlanConnect()
{
    ...
    lRetVal = sl_WlanConnect((signed char*)SSID_NAME, strlen(SSID_NAME), 0, 0, 0);
    ASSERT_ON_ERROR(lRetVal);
