...
int initMPU(uint32_t imu_index)
{
	...
	suit_i2c_read(MPU_ADDRESS, WHO_AM_I_MPU9250, rxBuff, 1);
	...
	//Reset device
	txBuff[0] = 0x80;
	suit_i2c_write(MPU_ADDRESS, PWR_MGMT_1, txBuff, 1);
	delay(100);
	...
	txBuff[0] = 0x03;
	suit_i2c_write(MPU_ADDRESS, CONFIG, txBuff, 1);

	//Set sensor output rate to 200 kHz, remaining consistent with filter update rate
	txBuff[0] = 0x04;
	suit_i2c_write(MPU_ADDRESS, SMPLRT_DIV, txBuff, 1);

	//Get gyro config register contents
	suit_i2c_read(MPU_ADDRESS, GYRO_CONFIG, rxBuff, 1);

	//Clear Fchoice and full scale bits
	txBuff[0] = rxBuff[0] & 0xE4;
	suit_i2c_write(MPU_ADDRESS, GYRO_CONFIG, txBuff, 1);
	...
	suit_i2c_read(MPU_ADDRESS, ACCEL_CONFIG, rxBuff, 1);

	//Clear full scale bits
	txBuff[0] = rxBuff[0] & 0xE7;
	suit_i2c_write(MPU_ADDRESS, ACCEL_CONFIG, txBuff, 1);
	...
	suit_i2c_read(MPU_ADDRESS, ACCEL_CONFIG2, rxBuff, 1);

	//Clear Fchoice and data_I2C rate bits
	txBuff[0] = rxBuff[0] & ~0xF;
	suit_i2c_write(MPU_ADDRESS, ACCEL_CONFIG2, txBuff, 1);
	...
	txBuff[0] = 0x12;
	suit_i2c_write(MPU_ADDRESS, INT_PIN_CFG, txBuff, 1);

	//Enable data_I2C ready interrupt
	txBuff[0] = 0x01;
	suit_i2c_write(MPU_ADDRESS, INT_ENABLE, txBuff, 1);
...
int initCompass(uint32_t imu_index)
{
	...
	suit_i2c_read(AK8963_ADDRESS, WHO_AM_I_AK8963, rxBuff, 1);
	...
	suit_i2c_read(AK8963_ADDRESS, AK8963_ASAX, rxBuff, 1);
	mAdj_X = (float) (rxBuff[0] - 128) / 256. + 1.;

	suit_i2c_read(AK8963_ADDRESS, AK8963_ASAY, rxBuff, 1);
	mAdj_Y = (float) (rxBuff[0] - 128) / 256. + 1.;

	suit_i2c_read(AK8963_ADDRESS, AK8963_ASAZ, rxBuff, 1);
		...
		float * gyro_X, float * gyro_Y, float * gyro_Z, uint32_t imu_index)
{
	...
	suit_i2c_read(MPU_ADDRESS, INT_STATUS, rxBuff, 1);

	if (rxBuff[0] == 0x01)
	{
		suit_i2c_read(MPU_ADDRESS, ACCEL_XOUT_H, rxBuff, 1);
		i2c_data = rxBuff[0];
		i2c_data <<= 8;

		suit_i2c_read(MPU_ADDRESS, ACCEL_XOUT_L, rxBuff, 1);
		i2c_data |= rxBuff[0];

		x = (float) i2c_data * aRes;

		suit_i2c_read(MPU_ADDRESS, ACCEL_YOUT_H, rxBuff, 1);
		i2c_data = rxBuff[0];
		i2c_data <<= 8;

		suit_i2c_read(MPU_ADDRESS, ACCEL_YOUT_L, rxBuff, 1);
		i2c_data |= rxBuff[0];

		y = (float) i2c_data * aRes;

		suit_i2c_read(MPU_ADDRESS, ACCEL_ZOUT_H, rxBuff, 1);
		i2c_data = rxBuff[0];
		i2c_data <<= 8;
		...
		*accel_Z = z;
		...
		suit_i2c_read(MPU_ADDRESS, GYRO_XOUT_H, rxBuff, 1);
		i2c_data = rxBuff[0];
		i2c_data <<= 8;

		suit_i2c_read(MPU_ADDRESS, GYRO_XOUT_L, rxBuff, 1);
		i2c_data |= rxBuff[0];

		x = (float) i2c_data * gRes - gyroOffsetX;

		suit_i2c_read(MPU_ADDRESS, GYRO_YOUT_H, rxBuff, 1);
		i2c_data = rxBuff[0];
		i2c_data <<= 8;

		suit_i2c_read(MPU_ADDRESS, GYRO_YOUT_L, rxBuff, 1);
		i2c_data |= rxBuff[0];

		y = (float) i2c_data * gRes - gyroOffsetY;

		suit_i2c_read(MPU_ADDRESS, GYRO_ZOUT_H, rxBuff, 1);
		i2c_data = rxBuff[0];
		i2c_data <<= 8;

		suit_i2c_read(MPU_ADDRESS, GYRO_ZOUT_L, rxBuff, 1);
		i2c_data |= rxBuff[0];
...
int readCompassData(float * mag_X, float * mag_Y, float * mag_Z, uint32_t imu_index) {
	...
	suit_i2c_read(AK8963_ADDRESS, AK8963_ST1, rxBuff, 1);

	//If new data_I2C is present, print out new data_I2C
	if (rxBuff[0] & 0x01)
	{
		//Read compass data_I2C then convert to milliGauss
		suit_i2c_read(AK8963_ADDRESS, AK8963_XOUT_H, rxBuff, 1);
		i2c_data = rxBuff[0];
		i2c_data <<= 8;

		suit_i2c_read(AK8963_ADDRESS, AK8963_XOUT_L, rxBuff, 1);
		i2c_data |= rxBuff[0];

		x = (float) i2c_data * mRes * mAdj_X + mUserX;

		suit_i2c_read(AK8963_ADDRESS, AK8963_YOUT_H, rxBuff, 1);
		i2c_data = rxBuff[0];
		i2c_data <<= 8;

		suit_i2c_read(AK8963_ADDRESS, AK8963_YOUT_L, rxBuff, 1);
		i2c_data |= rxBuff[0];

		y = (float) i2c_data * mRes * mAdj_Y + mUserY;

		suit_i2c_read(AK8963_ADDRESS, AK8963_ZOUT_H, rxBuff, 1);
		i2c_data = rxBuff[0];
		i2c_data <<= 8;

		suit_i2c_read(AK8963_ADDRESS, AK8963_ZOUT_L, rxBuff, 1);
		i2c_data |= rxBuff[0];

		z = (float) i2c_data * mRes * mAdj_Z + mUserZ;

		suit_i2c_read(AK8963_ADDRESS, AK8963_ST2, rxBuff, 1);
